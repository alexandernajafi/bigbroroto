...
for i = 1:nbrofvalues
    ...
    pairA = sum(GA(10:502).*exp(-1*j*w(10:502)'*cos(tempAngle)*Fs*d/v)); %Top horizontal pair
    pairB = sum(GB(10:502).*exp(-1*j*w(10:502)'*sin(tempAngle)*Fs*d/v)); %Right vertical pair
    pairC = sum(GC(10:502).*exp(-1*j*w(10:502)'*cos(tempAngle)*Fs*d/v)); %Bottom horizontal pair
    pairD = sum(GD(10:502).*exp(-1*j*w(10:502)'*sin(tempAngle)*Fs*d/v)); %Left vertical pair
